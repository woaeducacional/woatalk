...
interface MissionGroupsFlowProps {
  phaseId: number;
  onStartGroup: (groupIndex: number, isRedo?: boolean) => void;
}
...
function GroupCard({ group, idx, isCompleted, isLocked, canStart, onClick }: {
...
export const MissionGroupsFlow: React.FC<MissionGroupsFlowProps> = ({ phaseId, onStartGroup }) => {
  ...
  const handleGroupClick = (groupIndex: number) => {
    ...
    const isRedo = completedGroups.has(groupIndex);
    setCurrentGroup(groupIndex);
    onStartGroup(groupIndex, isRedo);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {MISSION_GROUPS.slice(0, 3).map((group, idx) => {
            ...
            return <GroupCard key={idx} group={group} idx={idx} isCompleted={isCompleted} isLocked={isLocked} canStart={canStart} onClick={() => handleGroupClick(idx)} />;
          })}
        </div>
        ...
        <div className="flex gap-6 justify-center">
          {MISSION_GROUPS.slice(3).map((group, i) => {
            ...
            const canStart = !isLocked && !isCompleted && (idx === 0 || completedGroups.has(idx - 1));
            return (
              <div key={idx} className="w-[calc(33.333%-12px)]" style={{ minWidth: '200px' }}>
                <GroupCard group={group} idx={idx} isCompleted={isCompleted} isLocked={isLocked} canStart={canStart} onClick={() => handleGroupClick(idx)} />
              </div>
            );
          })}
        </div>
