...
export const Activity4SpeakOnly: React.FC<Activity4Props> = ({ selectedSentences, onComplete }) => {
  const SENTENCES = [
  ...
  const handleNext = () => {
    ...
    if (accuracy >= 70) {
      ...
      if (currentSentenceIdx < 1) {
        ...
        onComplete(20); // 20 XP for Activity 4 (Speak Only)
      }
    } else {
    ...
    <div className="space-y-6">
      ...
      {isCompleted && currentSentenceIdx >= 1 && (
        <div className="rounded-lg border border-green-400/40 p-6 text-center" style={{ background: 'rgba(0,100,0,0.2)' }}>
          ...
          <p className="text-xl font-bold text-green-300 mb-2">Perfect!</p>
          <p className="text-sm text-green-200/70 mb-4">You spoke both sentences perfectly!</p>
          <button
